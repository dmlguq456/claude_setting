...
10. **post-it stale 후보 (시간 lifecycle 플래깅)** — Hermes Curator 벤치마킹 이식(T3). 작업장 repo 의 `.claude_reports/post-it.md` 들에서, time-sensitive 항목(`[in-progress/blocked YYYY-MM-DD]`·`Decisions YYYY-MM-DD:`)의 날짜가 **≥30d 미갱신이고 최근 git 활동과 단절**된 것 → "post-it stale 후보" 로 repo·항목·경과일 보고. ≥90d 는 "archive 대상" 표기. **제거하지 않는다 — 플래깅만**(sweep 은 사용자 흐름 또는 handoff 자리에서). 확실한 것 없으면 항목 생략. (post-it 정리 규칙 = [post-it SKILL](../skills/post-it/SKILL.md) sweep 절.) _※ store working tier 는 `WORKING_TTL_DAYS`(현재 21d)로 자동 만료 — post-it 의 30d/90d 플래깅과 동류(시간 기반 working lifecycle)이되 임계값·동작(플래깅 vs 자동만료)이 다름._
