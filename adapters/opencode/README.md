...
| tracked/untracked signal | `/track` semantics and `utilities/workflow-guard-hook.sh`; run `adapters/opencode/bin/preflight.sh mode [cwd] [session-id]` when no automatic prompt hook is attached |
