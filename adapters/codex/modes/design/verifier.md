...
2. **visual-harness console report** — 콘솔 로그·에러 수집. **에러 1 개라도 = Layer-1 hard fail** (깨진 화면).
3. **visual-harness screenshot inspection** — 캡처 후 이미지를 _직접 본다_ (자기 컨텍스트 안 — 메인에는 반환 X). 인터랙션·상태가 있으면 `steps[]` 로 전/후 캡처. 큰 화면은 `clip` crop.
4. **visual-harness DOM evidence** (필요 시) — 의심 지점을 수치로 확인: `getComputedStyle` 대비, 요소 box 겹침, 잘림(scrollWidth>clientWidth), 빈 컨테이너 등.
...
|---|---|---|
| `console.errors_zero` | 콘솔 에러·pageerror·네트워크 실패 0건 | `adapters/codex/bin/preflight.sh visual-harness <file.html>` 또는 adapter equivalent (Bash, headless Chromium via playwright — postwrite hook 동일 스크립트; exit 2 = 에러 있음). console/page-error evidence is reported by the Codex visual harness |
| `layout.no_overflow` | 요소가 컨테이너 밖으로 넘침 없음 | rendered evidence로 `adapters/codex/bin/preflight.sh visual-harness <file.html>` getBoundingClientRect; visual-harness = (a) 번들 `measure.mjs` (playwright getBoundingClientRect 직접 실행) 또는 (b) 정적 HTML 검사 (obvious inline `overflow:hidden` 깨짐·unclosed tag 등) |
| `layout.no_overlap` | 의도치 않은 요소 겹침 없음 | 동상 — visual-harness DOM evidence 또는 bundled measure.mjs / 정적 검사 |
| `layout.no_zero_box` | 0px 높이·빈 컨테이너 없음 | 동상 — `height:0`·`display:none` 의심 정적 grep 또는 visual-harness DOM evidence |
| `components.token_contract` | inline hex/px 로 토큰 재정의 없음; globals.css/tokens.css 대비 확인 | 파일 grep (DESIGN_PRINCIPLES §9) — adapter wrapper only |

> `color.contrast_pass` (본문 ≥4.5:1) 및 `typography.scale_pass` (슬라이드 본문 ≥24px / 모바일 hit-target ≥44px) 는 렌더 가능 시 `getComputedStyle` 로 결정론 판정; 렌더 불가 시 `unavailable` 강등 (hard fail 아님) — 아래 visual-harness 폴백 사다리 참조.
...
- **스크린샷·이미지를 메인 컨텍스트에 반환 금지.** verifier 는 screenshot inspection 를 자기 컨텍스트 안에서 본다 — 메인에는 텍스트만 돌아간다.
