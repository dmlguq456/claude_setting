...
| headless dispatch | Tool-contract check: `adapters/codex/bin/preflight.sh headless --check <worktree>` verifies the worktree and `codex exec` availability. Full unattended dispatch still requires an adapter-native transcript liveness mapping |
