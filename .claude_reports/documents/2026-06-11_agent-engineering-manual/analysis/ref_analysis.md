...
- 매핑 축 (원칙 → 실물):
  ...
  - P8 (상태 영속성·산출물 소통) → `.claude_reports` 통신 버스, plan/dev_logs 핸드오프, pipeline_state.yaml 재개, 3-tier (T1 사용자/T3 기계), worklog 2-layer — **한 줄기로 서술** (directive §2)
...
- 근거: worklog-board `.claude_reports/spec/prd.md` + `notes/` 실물 (cards/digests/duty/_layer2) + autopilot-note 2-layer 라우팅.
