...
### D5. 하네스 주입
세션 시작 주입은 Claude Code가 `projects/<cwd>/memory/`에서 함(우리가 못 바꿈). store에서 *현 cwd 해당분(working+durable/project) + global*을 projection 생성 → 기존 주입 활용.
...
mem_project(cwd)                       # store → projects/<cwd>/memory/ (주입용)
...
```mermaid
flowchart TD
  subgraph WRITE[자동 write - 전 tier]
    S[세션 맥락/결정] --> GATE
    OC[oncall self-review] --> GATE
    ADD[mem add/note] --> GATE
  end
  GATE[품질게이트·dedup·injection] --> SRC[(memory/ markdown<br/>원본·추적·포터블)]
  SRC -->|mem index| IDX[(.index.db FTS5)]
  SRC -->|mem project| PROJ[projects/cwd/memory/<br/>주입용]
  PROJ --> CC[Claude Code 세션 주입]
  R[mem recall] --> IDX
  R -.fallback.-> SRC
  LC[mem lifecycle] -->|working 만료·졸업 / durable consolidate| SRC
  subgraph MIG[이주]
    PI[post-it.md] --> M[mem migrate]
    AM[projects/*/memory 59파일] --> M
  end
  M --> SRC
```
...
- **D-4 (projection 갱신)**: 세션 시작 hook + oncall 보정.
