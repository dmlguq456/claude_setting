...
cwd 에 `.claude_reports/spec/<project>/` 가 있는 _진행 중 프로젝트_ 의 수정·기능 요청은 ad-hoc 직접 Edit 으로 끝내지 않는다. **순서 원칙: (기존 산출물 파악) → analyze → spec → dev.** 핵심 imperative:

- **새 세션 1 순위 = 기존 `.claude_reports/` 산출물 파악** — 손대기 전 cwd 의 `spec/<project>/prd.md` · `pipeline_state.yaml` · 최근 `plans/<project>/*` 를 _필요에 따라_ 먼저 읽어 프로젝트 상태를 잡는다. 맥락 모른 채 작업 X.
- **spec → dev 는 하드 원칙** — spec 없으면 `autopilot-spec` 먼저 유도 (throwaway 1 회성만 예외).
- **code 경유 _전_ spec-drift 사전 체크가 최우선** — spec 변경은 _항상_ **`autopilot-spec` update 모드** (prd.md 갱신 + `_internal/versions/v{N}/` versioning). drift 가 _명확_ 하면 자율 진행, **_애매_ 하면 사용자에 확인.** _"빨리"_ 로 건너뛰기 금지.
- **거의 모든 요청은 파이프 경유** — 작은 자연어 요청도 `--qa quick` (모든 모드 공통 경량 tier) 로 산출물 (plan/log) 남기며 진행. ad-hoc 직접 Edit 은 _순수 typo·1 줄 포맷_ 정도만.

상세 흐름 (기존 산출물 파악 → analyze 갱신 → spec-drift update → `autopilot-code` → `plans/<project>/`) = `WORKFLOW.md` §7. 새 세션도 도메인 트리거가 cwd `spec/*/pipeline_state.yaml` 감지해 발동.
...
| **spec-backed cwd / 사후 수정 요청** (cwd 에 `.claude_reports/spec/<project>/pipeline_state.yaml` 존재) | 본 문서 §9 + WORKFLOW §7 | 진입 시 **기존 `.claude_reports/` 산출물 파악 1 순위**. spec-first · drift-check-first. spec 변경은 autopilot-spec update 모드. 거의 모든 요청 `--qa quick` 파이프 (산출물 남김); ad-hoc 직접 Edit 은 순수 typo 만. |
