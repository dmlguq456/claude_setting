...
**원칙의 핵심**: agent 는 context 를 공유하는 것이 아니라 **파일 산출물로 소통한다.** 두 agent(또는 두 세션)가 직접 메모리를 주고받는 게 아니라, durable filesystem 에 산출물을 남기고 다음 agent 가 그것을 읽는다. amnesiac agent + durable filesystem 의 조합이다. 이것이 우리 세팅에서 `.agent_reports` 통신 버스로 구현된다 (2부 2.3).
...
| P8 | `.agent_reports` 통신 버스 → 핸드오프 → pipeline_state 재개 → 3-tier → headless 분사 → worklog 2-layer | `CONVENTIONS.md §5` · `§5.4` · `§5.8` · `§5.10` | 2.3 |
...
1부 P8 의 원칙("agent 는 context 를 공유하는 게 아니라 파일 산출물로 소통한다")이 우리 세팅에선 `.agent_reports` 통신 버스 한 줄기로 내려온다. 이 한 줄기가 1부 P8 → 2부 → 4부 를 관통하며, 매뉴얼에서 비중이 가장 크다.

**한 줄기 — `.agent_reports` 통신 버스**:
...
4. **headless 분사가 가능한 이유**: 위 세 가지가 갖춰지면 프로세스 경계를 산출물이 넘을 수 있다. headless 로 분사한 `claude -p` 프로세스가 같은 `.agent_reports` 트리에 산출물을 남기고, main 이 그것을 수확한다 (2.4 에서 상술). agent 가 amnesiac 이고 통신이 filesystem 경유이기 때문에, 프로세스가 분리돼도 통신이 끊기지 않는다.

5. **공유 트리 가드** (`CONVENTIONS.md §5.8`): 여러 worktree 가 하나의 canonical `.agent_reports` 를 symlink 공유할 때, spec 공유 단일파일(`prd.md`·`pipeline_state.yaml`·`pipeline_summary.md`) 동시 쓰기는 `.pipeline-lock` 으로 가드한다 (30분 stale override). `plans/<cycle>/` 는 사이클별 폴더라 경로 분리 → 비경합이다. 통신 버스가 동시성에서도 깨지지 않게 하는 층이다.
...
**왜 통합인가 (Tension — 흩어진 기억).** 이전엔 기억이 셋으로 흩어져 있었다 — post-it(`.agent_reports/post-it.md`, 프로젝트 단기) · auto-memory(`projects/<cwd>/memory/`, 프로젝트 장기) · user_profile(cross-project 프로필). 각자 별도 메커니즘·별도 위치라 (1) 한 검색면이 없고 (2) auto-memory 는 `.gitignore` 라 _들고 다닐 수 없었다_. → **하나의 store 로 통합**.
...
> 라이브 앵커: `tools/memory/mem.py`(구현·CLI `mem {add,note,recall,index,sync,inject,migrate,lifecycle,...}`) · `settings.json`(SessionStart/End hooks) · `CONVENTIONS.md §7`(canonical 규칙) · `.agent_reports/spec/prd.md`(청사진).
...
새 작업은 트랙별 첫 발화를 자연어 한 줄로 던지면 된다. 메인 Claude 가 WORKFLOW 작업-본질 매핑(`WORKFLOW.md §2`)으로 skill 을 고르고, 컨텍스트(cwd / `.agent_reports/` / 발화)를 보고 옵션을 자동 구성한 뒤 한 번 컨펌하고 invoke 한다 (`CLAUDE.md §0(B)`).
...
기대 산출물 — 수정 작업 기록은 `.agent_reports/plans/{date}_{slug}/` 에 떨어진다 (dev_logs·plan).
...
이 부는 산출물 기반 소통(1부 P8)의 종착점인 worklog-board 에이전틱 노트를 다룬다. 2부 2.3 의 `.agent_reports` 통신 버스에 산출물 md 가 쏟아지면, 사람이 그것을 하나하나 읽기 어렵다. worklog-board 의 제품 비전(v18)이 정확히 이 문제를 겨눈다 — "쏟아지는 산출물 md 를 하나하나 안 읽고도 따라가고 다시 찾는 것" (`worklog-board/.claude_reports/spec/prd.md §2`).
...
Layer 1 은 사용자가 보드에서 직접 만드는 프로젝트 관리·로깅 층(`kind: task` 단기 commitment / `kind: project` 납품·논문 출간 단위)이고 (`prd.md §2.1`·`§2.2`), Layer 2 는 에이전트가 `.agent_reports` 산출물을 정리한 산출물 맵이다 (`prd.md §2`).
...
worklog-board 의 2-layer 가 P8 산출물 소통 줄기의 종착점이다. 1부 P8 에서 원칙(amnesiac agent + durable filesystem)이 정의되고, 2부 2.3 에서 `.agent_reports` 통신 버스로 우리 세팅에 내려왔으며, 여기 4부에서 그 통신 버스에 쌓인 산출물을 사람이 다시 찾을 수 있게 정돈하는 worklog 2-layer 로 닫힌다.
