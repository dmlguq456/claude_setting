...
2. **🚦 작동 방식 — 📌tracked ↔ ⚡untracked** (_최상단 토대 섹션_) — hook 이 _신규 산출물 생성 순서_ 만 강제(신규 spec←research, plan←spec, 문서←research); 기존 편집·소스 코드는 convention. 두 모드 표 (**📌tracked** = 생성 순서 차단 + 매 프롬프트 라우팅 reminder / **⚡untracked** = 전부 우회·`/track`) + statusline(📌/⚡·git·context) 한 줄 + 한 줄 quote(편집은 소유 스킬 권장·convention). 단일 출처 = `hooks/artifact-guard.sh`·`utilities/workflow-guard-hook.sh`·`statusline.sh`·WORKFLOW.md §0(tracked 계약).
