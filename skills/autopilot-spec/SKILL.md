---
name: autopilot-spec
description: "_요구사항·청사진 작성_ 의 일반화 entry — 신규 의도 또는 기존 코드 정돈·공개 준비 자리 모두. mode 5종 (app / library / api / cli / research) + 다중 + auto. PRD 구조 = 공통 + mode 별 독립 섹션. autopilot-research / analyze-project 결과 자동 인용. analyze-project 의 _신규 의도 → 청사진_ 대칭 자리. 실제 코드 작업은 autopilot-code 가 담당 (spec/<name>/ 컨텍스트 자동 감지)."
...
Spec versioning 은 doc 트랙 ([CONVENTIONS §5.4.3](../../CONVENTIONS.md#5-skill-output-convention-3-tier-t1t2t3) — autopilot-refine 이 doc artifact 에 쓰는 `_internal/versions/v{N}/` 메커니즘 재사용). `prd.md` 가 _항상 최신_ (T1) — 사용자는 최신만 봄.
