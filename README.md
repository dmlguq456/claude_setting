...
| [`loops/README.md`](loops/README.md) | **상시 루프** (세션 밖 cron·headless) — `scout`(시간형 05:37 작업장 정찰·보고) · `note`(시간형 05:03 산출물 → worklog-board 노트화) · `golden/`(사건형 — 지침 수정 후 행동 회귀 테스트) · 후보 backlog. 출구는 모두 보고·제안까지 (merge 만 Claude 선별 책임 — CONVENTIONS §5.10) |
