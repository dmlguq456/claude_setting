...
| `audit` | **갈래 E** — multi-aspect 점검 + 기본 auto-fix chain. type 자동 인식 (plans/research/documents) + scope 자동 판단 (artifact 특성 기반). | `<artifact_path>` · `--scope auto(default)/facts/style/structure/cross-ref/coverage/all` (명시 = override 1순위) · `--read-only` · `--report-only` · `--no-fact-check` |
